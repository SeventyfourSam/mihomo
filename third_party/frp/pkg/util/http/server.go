...
func NewServer(cfg v1.WebServerConfig) (*Server, error) {
	// Embedded builds must not import net/http/pprof: its init registers
	// process-wide default HTTP handlers even when the admin server is off.
	if cfg.PprofEnable {
		return nil, errors.New("pprof is not available in embedded frp")
	}
