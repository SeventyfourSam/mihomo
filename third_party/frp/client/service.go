...
func init() {
	...
	// Embedded in mihomo: never change process-wide QUIC environment settings.
}
...
func (svr *Service) Run(ctx context.Context) error {
	...
	<-svr.ctx.Done()
	svr.stop()
	// Embedded callers reuse the process on reload. Join the reconnect worker
	// before returning, then close a controller published during cancellation.
	<-controllerDone
	svr.stop()
	return nil
}
...
func (svr *Service) stop() {
	...
	svr.ctlMu.Unlock()
	if ctl != nil {
		ctl.GracefulClose(svr.gracefulShutdownDuration)
		<-ctl.Done()
	}
}
